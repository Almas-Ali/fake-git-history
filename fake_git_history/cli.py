...
class FakeGitHistory:
    def __init__(self) -> None:
        ...
        self._start_date: datetime = datetime.now() - timedelta(days=90)
        self._end_date: datetime = datetime.now()
        self._start_time: str = "00:00:00"
        self._end_time: str = "23:59:59"
        self._time_zone: str = datetime.now(timezone.utc).astimezone().strftime("%z")
    ...
    def set_start_date(self, date: str) -> None:
        ...
        self._start_date = datetime.strptime(date, "%d/%m/%Y")
        self._log.primary(f"Start date set to: {self._start_date}")
    ...
    def set_start_time(self, time: str) -> None:
        ...
        self._log.primary(f"Start time set to: {self._start_time}")

    def set_end_time(self, time: str) -> None:
        """Set the end time (hh:mm:ss)."""
        self._end_time = time
        self._log.primary(f"End time set to: {self._end_time}")
    ...
    def get_random_time(self) -> str:
        ...
        start_time = datetime.strptime(self._start_time, time_format)
        end_time = datetime.strptime(self._end_time, time_format)
    ...
    def set_time_zone(self, time_zone: str) -> None:
        ...
        if re.match(r"[\+\-]\d{4}", time_zone):
            ...
            self._log.errormsg("Invalid time zone. Please provide a time zone in the format +0600.")
        self._log.primary(f"Time zone set to: {self._time_zone}")

    def set_commit_per_day(self, count_range: str) -> None:
        """Set the number of commits per day."""
        self._commit_per_day = count_range
        self._log.primary(f"Commits per day set to: {self._commit_per_day}")
    ...
    def get_commit_per_day(self, count_range: str) -> int:
        ...
        if re.match(r"\d+-\d+", count_range):
            _range_list: List[str] = count_range.strip().split("-")
            _internal_commit_per_day = random.randint(int(_range_list[0]), int(_range_list[1]))
        elif re.match(r"\d+", count_range):
            _internal_commit_per_day = int(count_range)
        else:
            self._log.errormsg("Invalid range. Please provide a range like 0-99999 or a single number.")

        self._log.primary(f"Commits per day set to: {_internal_commit_per_day}")
    ...
    def set_remote_origin(self, remote_origin: str) -> None:
        ...
        self._log.primary(f"Remote origin set to: {remote_origin}")
    ...
    def version(self) -> None:
        """Show the version."""
        rong.Text(
            text=f"Fake Git History version v{__version__}", fg=rong.ForegroundColor.GREEN, styles=[rong.Style.BOLD]
        ).print()
    ...
    def worker(self, filename: str, filecontent: str, adderstring: str, removerstring: str, state: int = 0) -> None:
        ...
        def add_file_commit() -> None:
            _ = subprocess.Popen(["git", "add", filename], stdout=subprocess.PIPE).communicate()

            _ = subprocess.Popen(["git", "commit", "-m", adderstring], stdout=subprocess.PIPE).communicate()

            _ = subprocess.Popen(
                ["git", "commit", "--amend", "--no-edit", f'--date="{_commit_date}"'], stdout=subprocess.PIPE
            ).communicate()
        ...
        def remove_file_commit() -> None:
            ...
            _ = subprocess.Popen(["git", "commit", "-m", removerstring], stdout=subprocess.PIPE).communicate()
    ...
    def run(self) -> None:
        ...
        self._log.green(f"Commits per day: {self._commit_per_day}")
        # self._log.warning(f"Commits: {self._commits}")

        _state: int = 0

        def update_state(state: int = 0) -> Tuple[int, Dict[str, str]]:
        ...
        if not os.path.exists(".git"):
            _ = subprocess.Popen(["git", "init"], stdout=subprocess.PIPE).communicate()
        ...
        try:
            ...
            while self._start_date <= self._end_date:
                ...
                _internal_commit_per_day: int = self.get_commit_per_day(self._commit_per_day)
                name_description: Dict[str, str] = self.name_maker()

                for i in range(_internal_commit_per_day):
                    # per date commit count
                    self._log.primary(f"Processing commit: {i + 1} of {_internal_commit_per_day}")
                    self.worker(
        ...
        if self._remote_origin != "":
            stdout, stderr = subprocess.Popen(
                ["git", "remote", "add", "origin", self._remote_origin], stdout=subprocess.PIPE
            ).communicate()
            if stderr:
                self._log.errormsg(f"Error: {str(stderr)}")
            if stdout:
                self._log.okmsg(f"Remote origin set to {self._remote_origin}")

        if self._auto_git_push:
            if self._remote_origin == "":
                self._log.errormsg("Remote origin not set. Please set the remote origin.")
                return
            stdout, stderr = subprocess.Popen(
                ["git", "push", "-u", "origin", "master"], stdout=subprocess.PIPE
            ).communicate()
            if stderr:
                self._log.errormsg(f"Error: {str(stderr)}")
            if stdout:
...
def main() -> None:
    ...
    parser.add_argument("--start-date", "-sd", type=str, help="Set the start date (dd/mm/yyyy)")
    parser.add_argument("--end-date", "-ed", type=str, help="Set the end date (dd/mm/yyyy)")
    parser.add_argument("--start-time", "-st", type=str, help="Set the start time (hh:mm:ss)")
    parser.add_argument("--end-time", "-et", type=str, help="Set the end time (hh:mm:ss)")
    parser.add_argument("--time-zone", "-tz", type=str, help="Set the time zone in the format +0600")
    parser.add_argument("--work-days-only", "-wd", action="store_true", help="Create commits to workdays only")
    parser.add_argument("--weekends-only", "-we", action="store_true", help="Create commits to weekends only")
    parser.add_argument("--commit-per-day", "-c", type=str, help="Set the number of commits per day")
    parser.add_argument("--auto-git-push", "-a", action="store_true", help="Enable auto git push")
    parser.add_argument("--remote-origin", "-r", type=str, help="Set the remote origin")
    parser.add_argument("--verbose", "-vv", action="store_true", help="Enable verbose mode")
    ...
    if args.commit_per_day:
        fgh.set_commit_per_day(count_range=str(args.commit_per_day))
